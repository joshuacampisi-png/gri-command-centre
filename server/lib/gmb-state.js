...
export function logPost(entry) {
  // Accepts the legacy shape { image, descriptionId, buttonUrl, buttonType, postedAt }
  // PLUS optional AI fields: generatedBy ('static-pool' | 'claude-sonnet-4-6' |
  // 'fal-nano-banana-pro+claude' | etc.), costUsd, fallbackReason, sourceMode,
  // category, hookText. Pass through so the dashboard's "Generated by" column
  // and AI cost totals work.
  const state = readState()
  state.posts.push({ ...entry, postedAt: entry.postedAt })
  writeState(state)
}
