/**
 * FINAL FIX: Create a custom section + custom template, push to theme + update page
 * Plus update body with both Mallu images
 */
...
const PACKING_IMG = 'https://cdn.shopify.com/s/files/1/0584/7410/2873/files/mallu-campisi-packing-warehouse.png?v=1779844129'

const themes = await rest('/themes.json')
const main = themes.themes.find(t => t.role === 'main')
console.log(`Theme: ${main.name} (${main.id})\n`)

// === 1. Create a fresh section file that renders page.content cleanly ===
const SECTION_LIQUID = `<style>
  .founders-page-wrap{max-width:1100px;margin:0 auto;padding:40px 20px;}
</style>
<div class="founders-page-wrap">
  {{ page.content }}
</div>
{% schema %}
{
...
{% endschema %}
`
console.log('Step 1: Create sections/founders-page.liquid')
const sec = await rest(`/themes/${main.id}/assets.json`, {
  method: 'PUT',
  body: JSON.stringify({ asset: { key: 'sections/founders-page.liquid', value: SECTION_LIQUID } })
})
console.log(sec.errors ? `  ✗ ${JSON.stringify(sec.errors)}` : '  ✓ Section created')
await sleep(800)

// === 2. Update templates/page.founders.json to use the new section ===
const TEMPLATE = {
  "sections": {
    "main": { "type": "founders-page", "settings": {} }
  },
  "order": ["main"]
}
console.log('\nStep 2: Update templates/page.founders.json')
const tmpl = await rest(`/themes/${main.id}/assets.json`, {
  method: 'PUT',
  body: JSON.stringify({ asset: { key: 'templates/page.founders.json', value: JSON.stringify(TEMPLATE, null, 2) } })
})
console.log(tmpl.errors ? `  ✗ ${JSON.stringify(tmpl.errors)}` : '  ✓ Template updated to reference our custom section')
await sleep(800)

// === 3. Build full body + update page ===
const SCHEMA = `<script type="application/ld+json">${JSON.stringify({
  "@context":"https://schema.org","@type":"AboutPage","url":"https://genderrevealideas.com.au/pages/about-the-founders","name":"About the Founders — Gender Reveal Ideas Australia",
  "mainEntity":{"@type":"Organization","name":"Gender Reveal Ideas Australia","foundingDate":"2010",
    "founder":[
...
<script type="application/ld+json">${JSON.stringify({
  "@context":"https://schema.org","@type":"Person","name":"Mallu Campisi","jobTitle":"Co-Founder, Gender Reveal Ideas Australia","image":[FAMILY_IMG,PACKING_IMG],"description":"Co-Founder of Australia's #1 gender reveal retailer. Mother of five and the creative force behind Gender Reveal Ideas Australia, founded in 2010. Personally tests every product and oversees fulfilment from the Bundall warehouse.","sameAs":["https://www.instagram.com/mallucampisi/"],"worksFor":{"@type":"Organization","name":"Gender Reveal Ideas Australia","url":"https://genderrevealideas.com.au","foundingDate":"2010","address":{"@type":"PostalAddress","streetAddress":"Unit 3B/13 Upton Street","addressLocality":"Bundall","addressRegion":"QLD","addressCountry":"AU"}}
})}</script>`

const BODY = `${SCHEMA}
<style>
.fp{font-family:system-ui,-apple-system,sans-serif;color:#1a1a1a;line-height:1.7}
.fp h1{font-size:clamp(28px,5vw,44px);line-height:1.1;margin:24px 0 12px;font-weight:700;letter-spacing:-0.01em;text-align:center}
...
.fp .lead{font-size:18px;color:#555;margin:6px auto 24px;max-width:680px;text-align:center}
.fp .badge{display:inline-block;padding:6px 14px;background:#fff7f9;color:#c44569;border-radius:100px;font-size:11px;letter-spacing:2px;text-transform:uppercase;font-weight:700}
...
<div style="text-align:center;padding:8px 0"><span class="badge">Meet the Founders</span></div>
...
<div class="photo-row">
  <div class="photo"><img src="${FAMILY_IMG}" alt="Mallu Campisi, Co-Founder of Gender Reveal Ideas Australia, with her five children" loading="eager"><span class="tag">Mallu + the family</span></div>
  <div class="photo"><img src="${PACKING_IMG}" alt="Mallu Campisi packing orders at the Gender Reveal Ideas warehouse in Bundall Gold Coast" loading="eager"><span class="tag">Mallu at the warehouse</span></div>
</div>
...
<p>What started as one Aussie mum has grown into Australia's #1 gender reveal retailer — shipping <strong>600,000+ units</strong> in the past 12 months alone to families in every Australian state and territory.</p>
<p>Mallu still personally tests every product and oversees fulfilment from the Bundall warehouse — the same warehouse you see in the photo above. <em>This isn't a faceless ecommerce brand.</em> It's an Australian mum-led business where the founder still packs boxes.</p>
...
<p>Co-Founder Josh Campisi works alongside Mallu to scale Gender Reveal Ideas Australia. Operations, supply chain, regulatory compliance, and the launch of the unique <a href="https://genderrevealideas.com.au/products/tnt-gender-reveal-australia">TNT Self-Hire Rental</a> all sit under his leadership.</p>
...
<div class="quote">Gender Reveal Ideas is "the one Australian brand large enough, old enough and accountable enough" that families can trust without learning complex supply chain details.<cite>— Hannah Whitfield, Consumer Editor, Mummy Time (18 May 2026)</cite></div>
...
<p>Founded on the Gold Coast in <strong>2010</strong>, GRI began as a family-owned business with one mission: make every gender reveal in Australia magical, safe, and unforgettable.</p>
<ul>
...
console.log('\nStep 3: Update page body + assign template_suffix: founders')
...
console.log(upd.errors ? `  ✗ ${JSON.stringify(upd.errors)}` : `  ✓ Page updated. Body: ${upd.page.body_html.length} chars. Template: ${upd.page.template_suffix}`)

await sleep(2000)
console.log('\nStep 4: HTTP verification')
const r = await fetch('https://genderrevealideas.com.au/pages/about-the-founders?v=' + Date.now())
console.log(`  Status: ${r.status}`)
const html = await r.text()
console.log(`  Contains "Mallu Campisi": ${html.includes('Mallu Campisi') ? '✓' : '✗'}`)
console.log(`  Contains "founders-page-wrap": ${html.includes('founders-page-wrap') ? '✓ (custom section is rendering!)' : '✗'}`)
console.log(`  Contains "GENDER REVEAL IDEAS FAQ": ${html.includes('GENDER REVEAL IDEAS FAQ') ? '⚠ FAQ still present' : '✓ no FAQ'}`)
console.log(`  Contains packing image URL: ${html.includes('mallu-campisi-packing-warehouse') ? '✓' : '✗'}`)
process.exit(0)
