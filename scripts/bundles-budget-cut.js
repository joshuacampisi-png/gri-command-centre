/**
 * Cut PMAX Bundles daily budget from $75 → $60 (-20%)
 * Snapshot before, mutate, verify, save rollback log.
 *
 * Rollback: node scripts/bundles-budget-cut.js --rollback
 */
...
const SNAPSHOT_DIR = 'data/snapshots/bundles-budget-cut'
mkdirSync(SNAPSHOT_DIR, { recursive: true })
const LOG = `${SNAPSHOT_DIR}/log.json`

const BUDGET_ID = '14807748208'
const NEW_BUDGET_MICROS = 60_000_000  // $60

const ROLLBACK = process.argv.includes('--rollback')

console.log(`\n=== ${ROLLBACK?'ROLLBACK':'CUT'} — PMAX Bundles daily budget ===\n`)
...
if (ROLLBACK) {
  ...
  const u = await customer.campaignBudgets.update([{
...
console.log(`Cut:    -$${(previousMicros-NEW_BUDGET_MICROS)/1e6}/day (-${Math.round((previousMicros-NEW_BUDGET_MICROS)/previousMicros*100)}%)`)
...
writeFileSync(LOG, JSON.stringify({
  ...
  rollbackCommand: 'node scripts/bundles-budget-cut.js --rollback',
}, null, 2))
console.log(`Log: ${LOG}`)
console.log(`Rollback: node scripts/bundles-budget-cut.js --rollback`)
process.exit(0)
