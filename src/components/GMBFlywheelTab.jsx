      ...
      <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
        ...
        <thead>
          <tr style={{ background: "var(--bg-canvas)" }}>
            {["Image", "Description ID", "Topic", "Generated by", "Button URL", "Posted at", "Status"].map(h => (
        ...
        <tbody>
          {posts.length === 0 ? (
            <tr><td colSpan={7} style={{ padding: "28px 12px", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>No posts in the last 30 days</td></tr>
          ) : posts.map(p => {
            ...
            const topic = p.trendingKeyword || p.category || null;
            const topicIsTrending = !!p.trendingKeyword;
            return (
              <tr key={p.id} style={{ borderTop: "1px solid var(--border)" }}>
                ...
                <td
                  style={{ padding: "8px 12px", fontSize: 12, color: topicIsTrending ? "var(--text)" : "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                  title={topic || ""}
                >
                  {topic ? (
...
/* ── Format helpers for trending data ── */
function formatSearchVolume(n) {
  const v = Number(n || 0);
  if (!v) return "—";
  if (v >= 1000) {
    const k = v / 1000;
...
function NextAiPostPanel({ aiMode, onPreview, onGenerate, busy }) {
  ...
  const trending = aiMode?.nextTrendingKeyword || null;
  const hasTrending = !!(trending && trending.keyword);

  // When trending data is available, show the richer "Trending Now" panel.
  if (hasTrending) {
    const wow = formatWowDelta(trending.wowDeltaPct);
    const lastPostedShort = formatShortDate(trending.lastPostedAt);
    const sourceLabel = trending.source || "Google Trends";

    const detailBits = [];
    if (trending.monthlySearches) {
      detailBits.push(`${formatSearchVolume(trending.monthlySearches)} searches/mo`);
    }
    if (wow) detailBits.push(`${wow} WoW`);
    detailBits.push(`from ${sourceLabel}`);

    const reasoning =
      trending.reasoning ||
      (lastPostedShort
        ? `Highest-volume keyword not posted about since ${lastPostedShort}`
        : "Highest-volume keyword not posted about yet");
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <div style={{ minWidth: 0, flex: "1 1 320px" }}>
            <div style={{
              fontSize: 11, color: "var(--green)", textTransform: "uppercase",
  ...
  // Fallback: legacy category-only panel.
...
/* ── Coming up (next trending candidates) ── */
function ComingUpSection() {
  ...
  const loadUpcoming = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await api("/trends/upcoming");
      const list = Array.isArray(r) ? r : (r?.items || r?.upcoming || []);
      setItems(list.slice(0, 10));
      setLoaded(true);
    } catch (err) {
      setError(err.message || "Could not load upcoming topics");
      setItems([]);
      setLoaded(true);
    } finally {
      setLoading(false);
  ...
  const handleToggle = () => {
    const next = !open;
    setOpen(next);
    if (next && !loaded && !loading) loadUpcoming();
  };

  const sorted = items
    ? [...items].sort((a, b) => {
        const av = Number(a?.monthlySearches || 0);
        const bv = Number(b?.monthlySearches || 0);
  ...
  return (
    <div style={{
      background: "var(--bg-surface)", border: "1px solid var(--border)",
      borderRadius: "var(--radius-lg)", overflow: "hidden", marginTop: 16,
    }}>
      <button
        ...
        <>
          ...
          {!loading && !error && sorted.length > 0 && (
            <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
              ...
              <thead>
                <tr style={{ background: "var(--bg-canvas)" }}>
                  <th style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border)" }}>#</th>
                  <th style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border)" }}>Keyword</th>
                  <th
                  ...
                  >
                    Searches/mo {sortDir === "desc" ? "↓" : "↑"}
                  </th>
                  <th style={{ padding: "10px 12px", textAlign: "right", fontSize: 11, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", borderBottom: "1px solid var(--border)" }}>WoW</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((row, i) => {
                  const wow = formatWowDelta(row?.wowDeltaPct);
                  const wowColor = row?.wowDeltaPct > 0
                    ? "var(--green)"
                    : row?.wowDeltaPct < 0
                      ? "var(--red)"
                      : "var(--text-muted)";
                  return (
                    <tr key={row?.id || row?.keyword || i} style={{ borderTop: "1px solid var(--border)" }}>
                      <td style={{ padding: "8px 12px", fontSize: 12, color: "var(--text-muted)", fontFamily: "var(--font-mono)", fontVariantNumeric: "tabular-nums" }}>
                        {i + 1}
                      </td>
                      <td style={{ padding: "8px 12px", fontSize: 13, color: "var(--text)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {row?.keyword || "—"}
                      </td>
                      <td style={{ padding: "8px 12px", fontSize: 12, color: "var(--text)", textAlign: "right", fontFamily: "var(--font-mono)", fontVariantNumeric: "tabular-nums" }}>
                        {formatSearchVolume(row?.monthlySearches)}
                      </td>
                      <td style={{ padding: "8px 12px", fontSize: 12, color: wowColor, textAlign: "right", fontFamily: "var(--font-mono)", fontVariantNumeric: "tabular-nums" }}>
                        {wow || "—"}
                      </td>
                    </tr>
                  );
  ...
  return (
    <div className="page" style={{ padding: 0 }}>
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        {/* Section 5b: Coming up — next trending candidates (only meaningful in FAL mode) */}
        {isFalMode && <ComingUpSection />}
