...
function SignedContracts() {
  ...
  useEffect(() => {
    fetch('/api/balloons/contracts').then(r => r.json()).then(d => setContracts(d.contracts || [])).catch(() => {});
  }, []);
...
function Modal({ title, onClose, children }) {
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
...
function AddHireModal({ onClose, onAdd, loading }) {
  ...
  return (
    <Modal title="Lodge new hire" onClose={onClose}>
      ...
      {preview && (
        <div style={{ background: "var(--bg-canvas)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "8px 12px", marginBottom: 12, fontSize: 12, color: "var(--text-muted)" }}>
...
function ReturnModal({ hire, onClose, onDecide, loading }) {
  const fullBond = 200 * (hire.boxQty || 1);
  ...
  return (
    <Modal title={`Process Return — ${hire.orderNumber}`} onClose={onClose}>
      <p style={{ fontSize: 13, color: "var(--text-muted)", margin: "0 0 14px" }}>
        ...
        {(hire.boxQty || 1) > 1 && <> ({hire.boxQty} boxes × $200)</>}
      </p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
        <button onClick={() => { setMode('full'); setPartialAmount(""); }} style={{ padding: '12px 8px', borderRadius: 'var(--radius)', cursor: 'pointer', border: mode === 'full' ? '2px solid var(--green)' : '1px solid var(--border)', background: mode === 'full' ? 'var(--green-bg)' : 'transparent', color: 'var(--green)', fontWeight: 600, fontSize: 13 }}>
          Full Refund<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>${fullBond} → customer</div>
        </button>
        <button onClick={() => setMode('partial')} style={{ padding: '12px 8px', borderRadius: 'var(--radius)', cursor: 'pointer', border: mode === 'partial' ? '2px solid var(--amber)' : '1px solid var(--border)', background: mode === 'partial' ? 'var(--amber-bg)' : 'transparent', color: 'var(--amber)', fontWeight: 600, fontSize: 13 }}>
          Partial<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>Keep some for damage</div>
        </button>
        <button onClick={() => { setMode('withhold'); setPartialAmount(""); }} style={{ padding: '12px 8px', borderRadius: 'var(--radius)', cursor: 'pointer', border: mode === 'withhold' ? '2px solid var(--red)' : '1px solid var(--border)', background: mode === 'withhold' ? 'var(--red-bg)' : 'transparent', color: 'var(--red)', fontWeight: 600, fontSize: 13 }}>
      ...
      {mode === 'partial' && (
        <div style={{ background: 'var(--amber-bg)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '12px 14px', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <input type="number" step="0.01" min="0" max={fullBond} autoFocus value={partialAmount}
              onChange={e => setPartialAmount(e.target.value)}
              placeholder={`up to ${fullBond}`}
              style={{ flex: 1, padding: '8px 10px', fontSize: 16, borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontWeight: 600, outline: 'none', background: 'var(--bg-surface)', color: 'var(--text)' }} />
          </div>
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <button onClick={onClose} style={btnBase}>Cancel</button>
        <button disabled={!mode || (mode === 'partial' && !partialAmount) || loading} onClick={submit} style={{ ...(mode === "withhold" ? btnDanger : btnPrimary), opacity: (!mode || (mode === 'partial' && !partialAmount) || loading) ? 0.5 : 1, cursor: mode && !loading ? "pointer" : "not-allowed" }}>{loading ? "Processing..." : "Confirm"}</button>
      </div>
    </Modal>
  );
}
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 20px" }}>
          ...
          {row("Boxes", `${hire.boxQty || 1} box${(hire.boxQty || 1) > 1 ? "es" : ""}`)}
          {row("Bond", `$${(hire.boxQty || 1) * 200}`)}
          {hire.bondPaymentId && row("Square ID", hire.bondPaymentId)}
          {hire.bondPaymentUrl && row("Pay link", <a href={hire.bondPaymentUrl} target="_blank" rel="noopener noreferrer" style={{ color: "var(--text)", textDecoration: "underline" }}>Open</a>)}
              ...
              <div style={{ marginTop: 6, padding: "10px 12px", background: "var(--amber-bg)", border: "1px solid var(--border)", borderRadius: "var(--radius)" }}>
                ...
                {hire.damageCharges.map((c, i) => (
                  <div key={i} style={{ fontSize: 12, color: "var(--text-soft)", marginBottom: 4, paddingBottom: 4, borderBottom: i < hire.damageCharges.length - 1 ? "1px dashed var(--border-strong)" : "none" }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-muted)" }}>{new Date(c.chargedAt).toLocaleString("en-AU", { timeZone: "Australia/Brisbane", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })} · {c.status || "COMPLETED"}</div>
                  </div>
                ))}
              </div>
            )}
...
export default function BalloonDashboard() {
  ...
  const loadHealth = useCallback(async () => {
    try {
      const res = await fetch("/api/balloons/health");
  ...
  const handleReturn = async (decision, opts = {}) => {
    ...
    try {
      ...
      const r = await api(`/${returnHire.id}/process-return`, { method: "POST", body: JSON.stringify(body) });
      if (decision === "refund") {
        if (r.manual) showToast("Marked refunded_manual — process in Square dashboard yourself", "warn");
      ...
      } else showToast("Bond withheld, customer notified");
      setReturnHire(null);
      await loadHires();
    } catch (err) {
  ...
  return (
    <div className="page" style={{ padding: 0 }}>
      ...
      <div style={{ padding: "14px 24px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontWeight: 600, fontSize: 15 }}>Helium Balloon Box Hire</span>
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            ...
            { label: "Awaiting bond", value: c.confirmed },
            { label: "Out on hire", value: c.active },
            ...
            <div key={s.label} style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "10px 14px", minWidth: 100 }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 600, color: "var(--text)", fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>{s.value}</div>
            </div>
          ))}
        </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 10 }}>
              {Object.entries(health.checks).map(([name, check]) => (
                <div key={name} style={{ fontSize: 12, padding: "8px 12px", borderRadius: "var(--radius-sm)", border: `1px solid var(--border)`, background: check.ok ? 'var(--green-bg)' : 'var(--red-bg)' }}>
                  <div style={{ fontWeight: 600, marginBottom: 4, textTransform: "uppercase", fontSize: 10, letterSpacing: "0.08em", color: check.ok ? "var(--green)" : "var(--red)" }}>
                    {check.ok ? "\u2713" : "\u2717"} {name}
                  </div>
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  ...
                  <tr style={{ borderTop: "1px solid var(--border)", cursor: "pointer" }} onClick={() => setDetailHire(hire)}>
                    <td style={{ padding: "10px 12px" }}>
                      <div style={{ fontSize: 13.5, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{hire.customerName}</div>
                    ...
                    <td style={{ padding: "10px 12px", fontSize: 12, color: "var(--text-muted)" }}>
                      {dates ? (
                        <>
                          ...
                          <div style={{ fontSize: 10, color: "var(--text-faint)" }}>Pickup: {dates.pickupFormatted}</div>
                        </>
                      ) : "N/A"}
                    </td>
                  ...
                  <tr style={{ borderBottom: "1px solid var(--border)", cursor: "pointer" }} onClick={() => setDetailHire(hire)}>
