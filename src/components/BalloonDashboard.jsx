  ...
  return (
    <Modal title={`Process Return — ${hire.orderNumber}`} onClose={onClose}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
        <button onClick={() => { setMode('full'); setPartialAmount(""); }} style={{ padding: '12px 8px', borderRadius: 8, cursor: 'pointer', border: mode === 'full' ? '2px solid #047857' : '1px solid var(--color-border-tertiary)', background: mode === 'full' ? '#DCFCE7' : 'transparent', color: '#047857', fontWeight: 700, fontSize: 13 }}>
          Full Refund<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>${fullBond} → customer</div>
        </button>
        <button onClick={() => setMode('partial')} style={{ padding: '12px 8px', borderRadius: 8, cursor: 'pointer', border: mode === 'partial' ? '2px solid #B45309' : '1px solid var(--color-border-tertiary)', background: mode === 'partial' ? '#FEF3C7' : 'transparent', color: '#92400E', fontWeight: 700, fontSize: 13 }}>
          Partial<div style={{ fontSize: 11, fontWeight: 400, marginTop: 2 }}>Keep some for damage</div>
        </button>
        <button onClick={() => { setMode('withhold'); setPartialAmount(""); }} style={{ padding: '12px 8px', borderRadius: 8, cursor: 'pointer', border: mode === 'withhold' ? '2px solid #A32D2D' : '1px solid var(--color-border-tertiary)', background: mode === 'withhold' ? '#FCEBEB' : 'transparent', color: '#A32D2D', fontWeight: 700, fontSize: 13 }}>
